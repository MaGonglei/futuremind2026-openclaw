...
describe("Feishu monitor status reporting", () => {
  ...
  it("reports connection mode and inbound activity through statusSink", async () => {
    setFeishuRuntime(
    ...
    expect(statusSink).toHaveBeenCalledWith({ mode: "websocket" });
    ...
    const onReadReceipt = handlers["im.message.message_read_v1"];
    if (!onReadReceipt) {
      throw new Error("missing im.message.message_read_v1 handler");
    }
